...
'''
package_scrap.py -- Thread to scrape information for a given package
...
from class_scrape import scrape_class


_STATUS_TEXT = '[ {status} ]\t{package}\t{misc}'
...
class PackageThread(Thread):
    ...
    '''Each thread defined here handles one package. It retrieves the
    web page, calculates the number of classes to parse, sequentially
    tries to retrieve each class' web page and parse it for its name,
    description, and methods.  This information is stored in an XML file
    named <package_name>.xml.
    '''
    ...
    def __init__(self, package_name, package_url):
        ...
        self.log_path = os.path.join('logs', package_name + '.log')
    ...
    def run(self):
        ...
        try:
            ...
            package_page = requests.get(self.package_url)
            ...
            statust = _STATUS_TEXT.format(status=_FAILURE,
                    package=self.package_name, misc=exc)
            ...
            for cls_ in cls_list:
                ...
                try:
                    ...
                    cls_url = get_absolute_url(self.package_url,
            ...
            misc = '%d/%d classes parsed.' % (successes, len(cls_list))
            if successes == len(cls_list):
                ...
                status = _PARTIAL
            statust = _STATUS_TEXT.format(status=status, package=self.package_name,
                    misc=misc)
...
def _main():
    ...
    name = 'java.awt'
    package = PackageThread(name, url)
